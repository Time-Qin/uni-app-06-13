	...
	<view>
		<uni-popup ref="popup3" type="bottom" >
			<view class="payContent">
						...
						<view class="content_view">
							<view class="content_text" v-for="(item,idx) in contentDatas.list" :key="item.id">
							 <view v-if="idx===changeIndex" class="text_main">
								<text class="iconfont icon-canju1 item">&nbsp; {{item.tableware}}</text>
								<text v-if="contentDatas.candle" class="iconfont icon-lazhu item">&nbsp; {{item.candle}}</text>
								<text class="iconfont icon-canju2 item">&nbsp; {{item.edible}}</text>
								<text class="iconfont icon-dangao item">&nbsp; {{item.size}}</text>
							 </view>
							</view>
						</view>
					</view>
				...
				<view class="four">
					<view class="left_button" @click="handleCarAdd">加入购物车</view>
